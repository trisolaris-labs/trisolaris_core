...
describe("Meta-Swap", async function () {
  ...
  describe("getTokenIndex", function () {
    ...
    it("Reverts when token address is not found", async function () {
      await expect(this.metaSwap.getTokenIndex(this.ZERO_ADDRESS)).to.be.revertedWith("Token does not exist");
    });
  });

  describe("getTokenBalance", function () {
    it("Returns correct balances of pooled tokens", async function () {
      expect(await this.metaSwap.getTokenBalance(0)).to.eq(BigNumber.from(String(1e18)));
      expect(await this.metaSwap.getTokenBalance(1)).to.eq(BigNumber.from(String(1e18)));
    });
  ...
  describe("addLiquidity", function () {
    it("Reverts when contract is paused", async function () {
      ...
      await expect(this.metaSwap.connect(this.user1).addLiquidity([String(1e18), String(3e18)], 0, this.MAX_UINT256)).to
        .be.reverted;

      // unpause
      await this.metaSwap.unpause();

      await this.metaSwap.connect(this.user1).addLiquidity([String(1e18), String(3e18)], 0, this.MAX_UINT256);
    ...
    it("Reverts with 'Cannot withdraw more than available'", async function () {
      await expect(
        this.metaSwap.connect(this.user1).calculateTokenAmount([this.MAX_UINT256, String(3e18)], false),
    ...
    it("Reverts with 'Must supply all tokens in pool'", async function () {
      ...
      await expect(
        this.metaSwap.connect(this.user1).addLiquidity([0, String(3e18)], this.MAX_UINT256, this.MAX_UINT256),
    ...
    it("Succeeds with correctly updated tokenBalance after imbalanced deposit", async function () {
      ...
      expect(await this.metaSwap.getTokenBalance(0)).to.eq(BigNumber.from(String(2e18)));
      expect(await this.metaSwap.getTokenBalance(1)).to.eq(BigNumber.from(String(4e18)));
    });
    ...
    it("Reverts when block is mined after deadline", async function () {
      ...
      await expect(
        this.metaSwap.connect(this.user1).addLiquidity([String(2e18), String(1e16)], 0, currentTimestamp + 60 * 5),
  ...
  describe("removeLiquidity", function () {
    it("Reverts with 'Cannot exceed total supply'", async function () {
      await expect(this.metaSwap.calculateRemoveLiquidity(this.MAX_UINT256)).to.be.revertedWith(
        "Cannot exceed total supply",
      );
    });

    it("Reverts with 'minAmounts must match poolTokens'", async function () {
      await expect(this.metaSwap.removeLiquidity(String(2e18), [0], this.MAX_UINT256)).to.be.revertedWith(
    ...
    it("Succeeds even when contract is paused", async function () {
      ...
      await this.metaSwap.connect(this.user1).addLiquidity([String(2e18), String(1e16)], 0, this.MAX_UINT256);
      ...
      await this.metaSwap.removeLiquidity(String(2e18), [0, 0], this.MAX_UINT256);
      await this.metaSwap.connect(this.user1).removeLiquidity(currentUser1Balance, [0, 0], this.MAX_UINT256);
    ...
    it("Succeeds with expected return amounts of underlying tokens", async function () {
      ...
      const [firstTokenBalanceBefore, secondTokenBalanceBefore, poolTokenBalanceBefore] = await getUserTokenBalances(
        this.user1,
        [this.ust, this.swapLPToken, this.metaSwapLPToken],
      );

      expect(poolTokenBalanceBefore).to.eq(BigNumber.from("1996275270169644725"));

      const [expectedFirstTokenAmount, expectedSecondTokenAmount] = await this.metaSwap.calculateRemoveLiquidity(
      ...
      await this.metaSwapLPToken.connect(this.user1).approve(this.metaSwap.address, poolTokenBalanceBefore);
      await this.metaSwap
        .connect(this.user1)
        .removeLiquidity(
          poolTokenBalanceBefore,
      ...
      const [firstTokenBalanceAfter, secondTokenBalanceAfter] = await getUserTokenBalances(this.user1, [
        this.ust,
        this.swapLPToken,
      ]);
    ...
    it("Reverts when minAmounts of underlying tokens are not reached due to front running", async function () {
      ...
      await this.metaSwap.connect(this.user2).addLiquidity([String(1e16), String(2e18)], 0, this.MAX_UINT256);

      // User 1 tries to remove liquidity which get reverted due to front running
      await this.metaSwapLPToken.connect(this.user1).approve(this.metaSwap.address, currentUser1Balance);
      await expect(
        this.metaSwap
          .connect(this.user1)
          .removeLiquidity(
    ...
    it("Reverts when block is mined after deadline", async function () {
      ...
      await this.metaSwapLPToken.connect(this.user1).approve(this.metaSwap.address, currentUser1Balance);
      await expect(
        this.metaSwap.connect(this.user1).removeLiquidity(currentUser1Balance, [0, 0], currentTimestamp + 60 * 5),
      ).to.be.revertedWith("Deadline not met");
    });
    ...
    it("Emits removeLiquidity event", async function () {
      ...
      await this.metaSwapLPToken.connect(this.user1).approve(this.metaSwap.address, currentUser1Balance);
      await expect(
        this.metaSwap.connect(this.user1).removeLiquidity(currentUser1Balance, [0, 0], this.MAX_UINT256),
  ...
  describe("removeLiquidityImbalance", function () {
    it("Reverts when contract is paused", async function () {
      ...
      await expect(
        this.metaSwap.removeLiquidityImbalance([String(1e18), String(1e16)], this.MAX_UINT256, this.MAX_UINT256),
      ).to.be.reverted;
    ...
    it("Reverts with 'Amounts should match pool tokens'", async function () {
      await expect(
        this.metaSwap.removeLiquidityImbalance([String(1e18)], this.MAX_UINT256, this.MAX_UINT256),
      ).to.be.revertedWith("Amounts should match pool tokens");
    });

    it("Reverts with 'Cannot withdraw more than available'", async function () {
      await expect(
        this.metaSwap.removeLiquidityImbalance([this.MAX_UINT256, this.MAX_UINT256], 1, this.MAX_UINT256),
      ).to.be.revertedWith("Cannot withdraw more than available");
    });
    ...
    it("Returns correct amount of burned lpToken", async function () {
      ...
      const burnedLPTokenAmount = await this.metaSwap
        .connect(this.user1)
        .callStatic.removeLiquidityImbalance([String(1e18), String(1e16)], currentUser1Balance, this.MAX_UINT256);
    ...
    it("Reverts when minAmounts of underlying tokens are not reached due to front running", async function () {
      ...
      // User 2 adds liquidity, which leads to change in balance of underlying tokens
      await this.metaSwap.connect(this.user2).addLiquidity([String(1e16), String(1e20)], 0, this.MAX_UINT256);
  ...
  describe("removeLiquidityOneToken", function () {
    it("Reverts when contract is paused.", async function () {
      ...
      await expect(this.metaSwap.removeLiquidityOneToken(String(2e18), 0, 0, this.MAX_UINT256)).to.be.reverted;
      await expect(
        this.metaSwap.connect(this.user1).removeLiquidityOneToken(currentUser1Balance, 0, 0, this.MAX_UINT256),
      ).to.be.reverted;
    });

    it("Reverts with 'Token index out of range'", async function () {
      await expect(this.metaSwap.calculateRemoveLiquidityOneToken(1, 5)).to.be.revertedWith("Token index out of range");
    });
    ...
    it("Reverts with 'Withdraw exceeds available'", async function () {
      ...
      await expect(this.metaSwap.calculateRemoveLiquidityOneToken(currentUser1Balance.mul(2), 0)).to.be.revertedWith(
    ...
    it("Succeeds with calculated token amount as minAmount", async function () {
      ...
      const calculatedFirstTokenAmount = await this.metaSwap.calculateRemoveLiquidityOneToken(currentUser1Balance, 0);
      ...
      await this.metaSwap
        .connect(this.user1)
        .removeLiquidityOneToken(currentUser1Balance, 0, calculatedFirstTokenAmount, this.MAX_UINT256);
    ...
    it("Returns correct amount of received token", async function () {
      await this.metaSwapLPToken.approve(this.metaSwap.address, this.MAX_UINT256);
      const removedTokenAmount = await this.metaSwap.callStatic.removeLiquidityOneToken(
        String(1e18),
        0,
        0,
        this.MAX_UINT256,
    ...
    it("Reverts when user tries to burn more LP tokens than they own", async function () {
      ...
      await expect(
        this.metaSwap.connect(this.user1).removeLiquidityOneToken(currentUser1Balance.add(1), 0, 0, this.MAX_UINT256),
      ).to.be.reverted;
    });
    ...
    it("Reverts when block is mined after deadline", async function () {
      ...
      await this.metaSwapLPToken.connect(this.user1).approve(this.metaSwap.address, currentUser1Balance);
      await expect(
        this.metaSwap.connect(this.user1).removeLiquidityOneToken(currentUser1Balance, 0, 0, currentTimestamp + 60 * 5),
      ).to.be.revertedWith("Deadline not met");
    });
  ...
  describe("swap", function () {
    it("Reverts when contract is paused", async function () {
      ...
      await expect(this.metaSwap.connect(this.user1).swap(0, 1, String(1e16), 0, this.MAX_UINT256)).to.be.reverted;
    });

    it("Reverts with 'Token index out of range'", async function () {
      await expect(this.metaSwap.calculateSwap(0, 9, String(1e17))).to.be.revertedWith("Token index out of range");
    });
    ...
    it("Succeeds with expected swap amounts", async function () {
      ...
      const calculatedSwapReturn = await this.metaSwap.calculateSwap(0, 1, String(1e17));
      ...
      await this.metaSwap.connect(this.user1).swap(0, 1, String(1e17), calculatedSwapReturn, this.MAX_UINT256);
    ...
    it("Reverts when minDy (minimum amount token to receive) is not reached due to front running", async function () {
      ...
      await expect(this.metaSwap.connect(this.user1).swap(0, 1, String(1e17), calculatedSwapReturn, this.MAX_UINT256))
        .to.be.reverted;
    });
    ...
    it("Returns correct amount of received token", async function () {
      const swapReturnAmount = await this.metaSwap.callStatic.swap(0, 1, String(1e18), 0, this.MAX_UINT256);
    ...
    it("Reverts when block is mined after deadline", async function () {
      ...
      await expect(
        this.metaSwap.connect(this.user1).swap(0, 1, String(1e17), 0, currentTimestamp + 60 * 5),
      ).to.be.revertedWith("Deadline not met");
    });

    it("Emits TokenSwap event", async function () {
      // User 1 initiates swap
      await expect(this.metaSwap.connect(this.user1).swap(0, 1, String(1e17), 0, this.MAX_UINT256)).to.emit(
        this.metaSwap,
        "TokenSwap",
      );
    });
  ...
  describe("swapUnderlying", function () {
    it("Reverts when contract is paused", async function () {
      ...
      // User 1 try to initiate swap
      await expect(this.metaSwap.connect(this.user1).swapUnderlying(0, 1, String(1e16), 0, this.MAX_UINT256)).to.be
        .reverted;
    });

    it("Reverts with 'Token index out of range'", async function () {
      await expect(this.metaSwap.calculateSwapUnderlying(0, 9, String(1e17))).to.be.revertedWith(
        "Token index out of range",
      );

      await expect(this.metaSwap.swapUnderlying(0, 9, String(1e17), 0, this.MAX_UINT256)).to.be.revertedWith(
        "Token index out of range",
      );
    });
    ...
    describe("Succeeds with expected swap amounts", function () {
      it("From 18 decimal token (meta) to 18 decimal token (base)", async function () {
        // User 1 calculates how much token to receive
        const calculatedSwapReturn = await this.metaSwap.calculateSwapUnderlying(0, 1, String(1e17));
    ...
    it("Reverts when minDy (minimum amount token to receive) is not reached due to front running", async function () {
      ...
      // User 2 swaps before User 1 does
      await this.metaSwap.connect(this.user2).swapUnderlying(0, 1, String(1e17), 0, this.MAX_UINT256);

      // User 1 initiates swap
      await expect(
        this.metaSwap.connect(this.user1).swapUnderlying(0, 1, String(1e17), calculatedSwapReturn, this.MAX_UINT256),
      ).to.be.reverted;
    });
    ...
    it("Succeeds when using lower minDy even when transaction is front-ran", async function () {
      ...
      await this.metaSwap
        .connect(this.user1)
        .swapUnderlying(0, 1, String(1e17), calculatedSwapReturnWithNegativeSlippage, this.MAX_UINT256);
    ...
    it("Returns correct amount of received token", async function () {
      const swapReturnAmount = await this.metaSwap.callStatic.swapUnderlying(0, 1, String(1e17), 0, this.MAX_UINT256);
    ...
    it("Reverts when block is mined after deadline", async function () {
      ...
      // User 1 tries swapping with deadline of +5 minutes
      await expect(
        this.metaSwap.connect(this.user1).swapUnderlying(0, 1, String(1e17), 0, currentTimestamp + 60 * 5),
      ).to.be.revertedWith("Deadline not met");
    });

    it("Emits TokenSwap event", async function () {
      // User 1 initiates swap
      await expect(this.metaSwap.connect(this.user1).swapUnderlying(0, 1, String(1e17), 0, this.MAX_UINT256)).to.emit(
        this.metaSwap,
        "TokenSwapUnderlying",
      );
    });
  ...
  describe("getVirtualPrice", function () {
    it("Returns expected value after initial deposit", async function () {
      expect(await this.metaSwap.getVirtualPrice()).to.eq(BigNumber.from(String(1e18)));
    });
    ...
    it("Returns expected values after swaps", async function () {
      ...
      expect(await this.metaSwap.getVirtualPrice()).to.eq(BigNumber.from("1000050005862349911"));

      await this.metaSwap.connect(this.user1).swap(1, 0, String(1e17), 0, this.MAX_UINT256);
      expect(await this.metaSwap.getVirtualPrice()).to.eq(BigNumber.from("1000100104768517937"));
    });

    it("Returns expected values after imbalanced withdrawal", async function () {
      await this.metaSwap.connect(this.user1).addLiquidity([String(1e18), String(1e18)], 0, this.MAX_UINT256);
      await this.metaSwap.connect(this.user2).addLiquidity([String(1e18), String(1e18)], 0, this.MAX_UINT256);
      expect(await this.metaSwap.getVirtualPrice()).to.eq(BigNumber.from(String(1e18)));
      ...
      expect(await this.metaSwap.getVirtualPrice()).to.eq(BigNumber.from("1000100094088440633"));
      ...
      expect(await this.metaSwap.getVirtualPrice()).to.eq(BigNumber.from("1000200154928939884"));
    });
    ...
    it("Value is unchanged after balanced deposits", async function () {
      ...
      await this.metaSwap.connect(this.user2).addLiquidity([String(2e18), String(0)], 0, this.MAX_UINT256);
      expect(await this.metaSwap.getVirtualPrice()).to.eq(BigNumber.from("1000167146429977312"));
      // User 2 makes balanced deposit, keeping the ratio 2:1
      await this.metaSwap.connect(this.user2).addLiquidity([String(2e18), String(1e18)], 0, this.MAX_UINT256);
      expect(await this.metaSwap.getVirtualPrice()).to.eq(BigNumber.from("1000167146429977312"));
    });
    ...
    it("Value is unchanged after balanced withdrawals", async function () {
      ...
      await this.metaSwap.connect(this.user1).removeLiquidity(String(1e18), ["0", "0"], this.MAX_UINT256);
      expect(await this.metaSwap.getVirtualPrice()).to.eq(BigNumber.from(String(1e18)));
    });
  });
  ...
  describe("setSwapFee", function () {
    it("Emits NewSwapFee event", async function () {
      await expect(this.metaSwap.setSwapFee(BigNumber.from(1e8))).to.emit(this.metaSwap, "NewSwapFee");
    });
    ...
    it("Reverts when called by non-owners", async function () {
      ...
      await expect(this.metaSwap.connect(this.user2).setSwapFee(BigNumber.from(1e8))).to.be.reverted;
    });

    it("Reverts when fee is higher than the limit", async function () {
      await expect(this.metaSwap.setSwapFee(BigNumber.from(1e8).add(1))).to.be.reverted;
    });
  ...
  describe("setAdminFee", function () {
    it("Emits NewAdminFee event", async function () {
      await expect(this.metaSwap.setAdminFee(BigNumber.from(1e10))).to.emit(this.metaSwap, "NewAdminFee");
    });

    it("Reverts when called by non-owners", async function () {
      await expect(this.metaSwap.connect(this.user1).setSwapFee(0)).to.be.reverted;
      await expect(this.metaSwap.connect(this.user2).setSwapFee(BigNumber.from(1e10))).to.be.reverted;
    });

    it("Reverts when adminFee is higher than the limit", async function () {
      await expect(this.metaSwap.setAdminFee(BigNumber.from(1e10).add(1))).to.be.reverted;
    });

    it("Succeeds when adminFee is within the limit", async function () {
      await this.metaSwap.setAdminFee(BigNumber.from(1e10));
      expect((await this.metaSwap.swapStorage()).adminFee).to.eq(BigNumber.from(1e10));
    });
  });
  ...
  describe("setFeeAddress", () => {
    ...
    it("Emits NewFeeAddress event", async function () {
      await expect(this.metaSwap.setFeeAddress(this.user2.address)).to.emit(this.metaSwap, "NewFeeAddress");
    });

    it("Reverts when called by non-owners", async function () {
      await expect(this.metaSwap.connect(this.user1).setFeeAddress(this.user2.address)).to.be.reverted;
      await expect(this.metaSwap.connect(this.user2).setFeeAddress(this.user2.address)).to.be.reverted;
    });
  ...
  describe("getAdminBalance", function () {
    it("Reverts with 'Token index out of range'", async function () {
      await expect(this.metaSwap.getAdminBalance(3)).to.be.revertedWith("Token index out of range");
    });
  ...
  describe("withdrawAdminFees", function () {
    ...
    it("Succeeds when there are no fees withdrawn", async function () {
      ...
      const [firstTokenBefore, secondTokenBefore] = await getUserTokenBalances(this.user1, [
        this.ust,
        this.swapLPToken,
      ]);
      ...
      const [firstTokenAfter, secondTokenAfter] = await getUserTokenBalances(this.user1, [this.ust, this.swapLPToken]);
    ...
    it("Succeeds with expected amount of fees withdrawn (swap)", async function () {
      ...
      const [firstTokenBefore, secondTokenBefore] = await getUserTokenBalances(this.user2, [
        this.ust,
        this.swapLPToken,
      ]);
      ...
      const [firstTokenAfter, secondTokenAfter] = await getUserTokenBalances(this.user2, [this.ust, this.swapLPToken]);
    ...
    it("Succeeds with expected amount of fees withdrawn (swapUnderlying)", async function () {
      ...
      // Sets adminFee to 1% of the swap fees
      await this.metaSwap.setAdminFee(BigNumber.from(10 ** 8));
      await this.metaSwap.connect(this.user1).swapUnderlying(0, 1, String(1e17), 0, this.MAX_UINT256);
      await this.metaSwap.connect(this.user1).swapUnderlying(1, 0, String(1e17), 0, this.MAX_UINT256);
    ...
    it("Withdrawing admin fees has no impact on users' withdrawal", async function () {
      ...
      await this.metaSwapLPToken.connect(this.user1).approve(this.metaSwap.address, user1LPTokenBalance);
      await this.metaSwap.connect(this.user1).removeLiquidity(user1LPTokenBalance, [0, 0], this.MAX_UINT256);
  ...
  describe("rampA", function () {
    ...
    it("Emits RampA event", async function () {
      await expect(this.metaSwap.rampA(100, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1)).to.emit(
        this.metaSwap,
        "RampA",
      );
    });
    ...
    it("Reverts when non-owner calls it", async function () {
      await expect(this.metaSwap.connect(this.user1).rampA(55, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1))
        .to.be.reverted;
    });

    it("Reverts with 'Wait 1 day before starting ramp'", async function () {
      await this.metaSwap.rampA(55, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1);
      await expect(this.metaSwap.rampA(55, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1)).to.be.revertedWith(
    ...
    it("Reverts with 'futureA_ must be > 0 and < MAX_A'", async function () {
      await expect(this.metaSwap.rampA(0, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1)).to.be.revertedWith(
        "futureA_ must be > 0 and < MAX_A",
      );
    });

    it("Reverts with 'futureA_ is too small'", async function () {
      await expect(this.metaSwap.rampA(24, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1)).to.be.revertedWith(
    ...
    it("Reverts with 'futureA_ is too large'", async function () {
      await expect(
        this.metaSwap.rampA(101, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1),
  ...
  describe("stopRampA", function () {
    it("Emits StopRampA event", async function () {
      // call rampA()
      await this.metaSwap.rampA(100, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 100);

      // Stop ramp
      await expect(this.metaSwap.stopRampA()).to.emit(this.metaSwap, "StopRampA");
    });
    ...
    it("Reverts with 'Ramp is already stopped'", async function () {
      ...
      await expect(this.metaSwap.stopRampA()).to.be.revertedWith("Ramp is already stopped");
    });
  });
  ...
  describe("Check for timestamp manipulations", function () {
    ...
    it("Check for maximum differences in A and virtual price when A is increasing", async function () {
      ...
      // Start ramp
      await this.metaSwap.rampA(100, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1);
    ...
    it("Check for maximum differences in A and virtual price when A is decreasing", async function () {
      ...
      // Start ramp
      await this.metaSwap.rampA(25, (await getCurrentBlockTimestamp()) + 14 * TIME.DAYS + 1);
    ...
    describe("Check for attacks while A is ramping upwards", () => {
      ...
      beforeEach(async function () {
        ...
        initialAttackerBalances = await getUserTokenBalances(attacker, [this.ust, this.swapLPToken]);
        ...
        initialPoolBalances = [await this.metaSwap.getTokenBalance(0), await this.metaSwap.getTokenBalance(1)];
      ...
      describe(
        ...
        () => {
          it("Attack fails with 900 seconds between blocks", async function () {
            ...
            await this.metaSwap.connect(attacker).swap(0, 1, String(1e18), 0, this.MAX_UINT256);
            const secondTokenOutput = (await getUserTokenBalance(attacker, this.swapLPToken)).sub(
            ...
            expect(await this.metaSwap.getTokenBalance(1)).to.be.eq("91408257454997694");
            ...
            await this.metaSwap.connect(attacker).swap(1, 0, secondTokenOutput, 0, this.MAX_UINT256);
            const firstTokenOutput = (await getUserTokenBalance(attacker, this.ust)).sub(balanceBefore);
            ...
            const finalAttackerBalances = await getUserTokenBalances(attacker, [this.ust, this.swapLPToken]);
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async function () {
            ...
            // Check for pool balance changes
            const finalPoolBalances = [await this.metaSwap.getTokenBalance(0), await this.metaSwap.getTokenBalance(1)];
      ...
      describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async function () {
            ...
            expect(await this.metaSwap.getTokenBalance(0)).to.be.eq(String(2e18));
            expect(await this.metaSwap.getTokenBalance(1)).to.be.eq("1988066748939318647");
