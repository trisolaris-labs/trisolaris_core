...
describe("Meta-Swap Deposit Contract", async function () {
  ...
  beforeEach(async function () {
    ...
    await asyncForEach([this.owner, this.user1, this.user2], async (signer) => {
      await asyncForEach(
        [this.ust, this.dai, this.usdt, this.metaSwapLPToken],
        async (token) => {
          await token
            .connect(signer)
            .approve(this.metaSwapDeposit.address, this.MAX_UINT256)
        },
      )
    })
  })
  ...
  describe("getToken", () => {
    ...
    it("Reverts if out of range", async function () {
      await expect(this.metaSwapDeposit.getToken(20)).to.be.revertedWith(
        "out of range",
      )
    })
  })
  ...
  describe("swap", () => {
    it("From 18 decimal token (meta) to 18 decimal token (base)", async function () {
      ...
      const calculatedSwapReturn = await this.metaSwapDeposit.calculateSwap(
        0,
        1,
        String(1e17),
      )
  ...
  describe("addLiquidity", () => {
    it("Reverts when deadline is not met", async function () {
      ...
      await expect(
        this.metaSwapDeposit.addLiquidity(
          tokenDepositAmounts,
          0,
          blockTimestamp - 100,
        ),
    ...
    it("Succeeds when depositing balanced amounts", async function () {
      ...
      const minToMint = await this.metaSwapDeposit.calculateTokenAmount(
        tokenDepositAmounts,
        true,
      )
      ...
      await this.metaSwapDeposit.addLiquidity(
        tokenDepositAmounts,
  ...
  describe("removeLiquidity", () => {
    beforeEach(async function () {
      ...
      await this.metaSwapDeposit.addLiquidity(
        [String(3e18), String(1e18), String(0)],
        0,
        this.MAX_UINT256,
      )
    })
    ...
    it("Reverts when minAmounts are not reached", async function () {
      ...
      await expect(
        this.metaSwapDeposit.removeLiquidity(
          String(1e18),
          [String(6e18), 0, 0],
          this.MAX_UINT256,
        ),
    ...
    it("Succeeds with expected minAmounts", async function () {
      const minAmounts = await this.metaSwapDeposit.calculateRemoveLiquidity(
        String(1e18),
      )
      ...
      await this.metaSwapDeposit.removeLiquidity(
        String(1e18),
        minAmounts,
        this.MAX_UINT256,
      )
  ...
  describe("removeLiquidityOneToken", () => {
    ...
    it("Reverts when minAmount is not reached", async function () {
      await expect(
        this.metaSwapDeposit.removeLiquidityOneToken(
          String(1e18),
          0,
          this.MAX_UINT256,
          this.MAX_UINT256,
    ...
    it("Succeeds when withdrawing via a meta level token", async function () {
      const minAmount = await this.metaSwapDeposit.calculateRemoveLiquidityOneToken(
        String(1e18),
        0,
      )
      const returnValue =
        await this.metaSwapDeposit.callStatic.removeLiquidityOneToken(
          String(1e18),
          0,
          minAmount,
          this.MAX_UINT256,
      ...
      await this.metaSwapDeposit.removeLiquidityOneToken(
        String(1e18),
        0,
        minAmount,
        this.MAX_UINT256,
  ...
  describe("removeLiquidityImbalance", () => {
    beforeEach(async function () {
      // Add more liquidity to test with
      await this.metaSwapDeposit.addLiquidity(
        [String(3e18), String(3e18), String(2e18)],
        0,
        this.MAX_UINT256,
      )
    })
    ...
    it("Reverts when slippage setting is 0%", async function () {
      ...
      const maxBurnAmount = await this.metaSwapDeposit.calculateTokenAmount(
        amounts,
        false,
      )
      await expect(
    ...
    it("Succeeds when only withdrawing meta-level tokens", async function () {
      ...
      // Apply 0.1% slippage
      const maxBurnAmount = (
        await this.metaSwapDeposit.calculateTokenAmount(amounts, false)
      )
        .mul(1001)
        .div(1000)
      ...
      // Perform the call
      const returnValues =
        await this.metaSwapDeposit.callStatic.removeLiquidityImbalance(
          amounts,
          maxBurnAmount,
          this.MAX_UINT256,
        )
      await this.metaSwapDeposit.removeLiquidityImbalance(
        amounts,
        maxBurnAmount,
        this.MAX_UINT256,
      )
      ...
      expect(balancesBefore[3].sub(balancesAfter[3])).to.eq(
        "1007410020583259754",
      )
