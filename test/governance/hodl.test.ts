...
describe('scenario:TreasuryVester', () => {
	...
	it('claim:fail not yet time', async () => {
		await expect(hodl.reclaimTokens(
			tri.address,
			hodlAmount,
			recepient.address
		)).to.be.revertedWith('Hodl:: reclaimTokens: not time yet')
	...
	it('claim:fail not owner', async () => {
		await expect(hodl.connect(recepient).reclaimTokens(
			tri.address,
			hodlAmount,
			recepient.address
	  ...
	  it('claim:all', async () => {
		...
		await hodl.connect(deployer).reclaimTokens(
			zeroAddress,
			ethAmount,
			recepient.address
		)
