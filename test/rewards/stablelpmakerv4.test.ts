...
describe("StableLpMaker - V4", function () {
  before(async function () {
    this.LPMakerV3 = await ethers.getContractFactory("StableLPMakerV4");
  ...
  beforeEach(async function () {
    ...
    const TOKEN_ADDRESSES = [this.usdc.address, this.usdt.address];
    const TOKEN_DECIMALS = [18, 18];
    this.LP_TOKEN_NAME = "Saddle USDC/USDT";
    ...
    this.lpMakerV4 = await this.LPMakerV3.connect(this.owner).deploy(
      this.swapFlashLoan.address,
      this.pTRI.address,
      this.usdc.address,
      this.usdt.address,
    ...
    await this.lpMakerV4.deployed();
    this.exploiter = await this.LPMakerV3ExploitMock.connect(this.owner).deploy(this.lpMakerV4.address);
    await this.exploiter.deployed();
    await this.swapFlashLoan.connect(this.owner).setFeeAddress(this.lpMakerV4.address);
    ...
    expect(await this.swapFlashLoan.feeAddress()).to.eq(this.lpMakerV4.address);
    ...
    await this.metaSwap.connect(this.owner).setFeeAddress(this.lpMakerV4.address);
    ...
    expect(await this.metaSwap.feeAddress()).to.eq(this.lpMakerV4.address);
  });

  it("should withdraw fees to stableLPMakerV3 from stableswap", async function () {
    expect(await this.ust.balanceOf(this.lpMakerV4.address)).to.equal("0");
    expect(await this.swapFlashLoan.feeAddress()).to.eq(this.lpMakerV4.address);
    await this.lpMakerV4.withdrawStableTokenFees(this.swapFlashLoan.address);
    expect(await this.ust.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usn.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usdt.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usdc.balanceOf(this.lpMakerV4.address)).to.equal(999993464094);
    // gets no fees now
    await this.lpMakerV4.withdrawStableTokenFees(this.swapFlashLoan.address);
    expect(await this.ust.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usn.balanceOf(this.lpMakerV4.address)).to.equal(0);
  ...
  it("should withdraw fees to stableLPMakerV3 from metastableswap", async function () {
    ...
    await this.lpMakerV4.withdrawStableTokenFees(this.metaSwap.address);
    expect(await this.ust.balanceOf(this.lpMakerV4.address)).to.equal(10019739648609);
    expect(await this.swapLPToken.balanceOf(this.lpMakerV4.address)).to.equal(9980241397654);
    expect(await this.usn.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usdt.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usdc.balanceOf(this.lpMakerV4.address)).to.equal(0);
    // gets no fees now
    await this.lpMakerV4.withdrawStableTokenFees(this.metaSwap.address);
    expect(await this.ust.balanceOf(this.lpMakerV4.address)).to.equal(10019739648609);
    expect(await this.swapLPToken.balanceOf(this.lpMakerV4.address)).to.equal(9980241397654);
  ...
  it("should remove liquidity of baseLps", async function () {
    ...
    await expect(this.lpMakerV4.removeLiquidity(this.swapFlashLoan.address)).to.be.revertedWith(
      "StableLPMaker: Stableswap not whitelisted",
    );
    await this.lpMakerV4.connect(this.owner).addStableSwap(this.swapFlashLoan.address);
    expect(await this.lpMakerV4.whitelistedStableSwapAddresses(this.swapFlashLoan.address)).to.equal(true);

    await this.lpMakerV4.removeLiquidity(this.swapFlashLoan.address);
    // swapLPToken is converted into base tokens
    expect(await this.ust.balanceOf(this.lpMakerV4.address)).to.equal(10019739648609);
    expect(await this.swapLPToken.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usn.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usdt.balanceOf(this.lpMakerV4.address)).to.equal(4991784072393);
    expect(await this.usdc.balanceOf(this.lpMakerV4.address)).to.equal(4988457485835);
  });
  ...
  it("should convert USDC to USDT ", async function () {
    ...
    expect(await this.usdc.balanceOf(this.lpMakerV4.address)).to.equal(999993464094);
    // should revert for not whitelisted stableswaps
    await expect(this.lpMakerV4.swapStableTokens(this.swapFlashLoan.address, 0, 1)).to.be.revertedWith(
      "StableLPMaker: Stableswap not whitelisted",
    );
    await this.lpMakerV4.connect(this.owner).addStableSwap(this.swapFlashLoan.address);
    expect(await this.lpMakerV4.whitelistedStableSwapAddresses(this.swapFlashLoan.address)).to.equal(true);
    ...
    await this.lpMakerV4.swapStableTokens(this.swapFlashLoan.address, 0, 1);
    expect(await this.ust.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usn.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usdc.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usdt.balanceOf(this.lpMakerV4.address)).to.equal(999906534642);
  });
  ...
  it("should convert UST to USDC ", async function () {
    ...
    // should revert for not whitelisted stableswaps
    await expect(this.lpMakerV4.swapStableTokens(this.metaSwapDeposit.address, 0, 1)).to.be.revertedWith(
      "StableLPMaker: Stableswap not whitelisted",
    );
    await this.lpMakerV4.connect(this.owner).addStableSwap(this.metaSwapDeposit.address);
    expect(await this.lpMakerV4.whitelistedStableSwapAddresses(this.metaSwapDeposit.address)).to.equal(true);
    await this.lpMakerV4.swapStableTokens(this.metaSwapDeposit.address, 0, 1);
    expect(await this.ust.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.swapLPToken.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usn.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usdt.balanceOf(this.lpMakerV4.address)).to.equal(4991784072393);
    expect(await this.usdc.balanceOf(this.lpMakerV4.address)).to.equal(14997686942394);
  });
  ...
  it("should add liquidity to stableswap", async function () {
    ...
    expect(await this.usdt.balanceOf(this.lpMakerV4.address)).to.equal(0);
    expect(await this.usdc.balanceOf(this.lpMakerV4.address)).to.equal(999993464094);
    await this.lpMakerV4.addLiquidityToStableSwap();
    expect(await this.swapLPToken.balanceOf(this.lpMakerV4.address)).to.equal(999949969420);
    expect(await this.usn.balanceOf(this.lpMakerV4.address)).to.equal(0);
  });
  ...
  it("should send assets to pTRI ", async function () {
    ...
    await this.lpMakerV4.addLiquidityToStableSwap();
    expect(await this.swapLPToken.balanceOf(this.lpMakerV4.address)).to.equal(999949969420);
    await this.lpMakerV4.sendLpToken();
    expect(await this.swapLPToken.balanceOf(this.pTRI.address)).to.be.closeTo("999949969420", 10);
  });

  it("should fail to send tlp to pTRI when not enough balance", async function () {
    await expect(this.lpMakerV4.sendLpToken()).to.be.revertedWith("StableLpMaker: no TLP to send");
  });
  ...
  it("should run all the steps together without converting assets", async function () {
    ...
    await this.lpMakerV4.convertStables([this.swapFlashLoan.address], [], [], [], []);
  ...
  it("should convert usdc, ust to usn, and perform all steps", async function () {
    ...
    expect(await this.swapLPToken.balanceOf(this.pTRI.address)).to.equal(0);
    await this.lpMakerV4.connect(this.owner).addStableSwap(this.swapFlashLoan.address);
    await this.lpMakerV4.connect(this.owner).addStableSwap(this.metaSwap.address);
    await this.lpMakerV4.connect(this.owner).addStableSwap(this.metaSwapDeposit.address);
    await this.lpMakerV4.convertStables(
    ...
    expect(await this.swapLPToken.balanceOf(this.dao.address)).to.be.closeTo("0", 10);
    expect(await this.swapLPToken.balanceOf(this.pTRI.address)).to.be.closeTo("20988886166238", 10);
  });

  describe("StableLPMakerV4: Dao Tests", function () {
    it("should have correct dao address", async function () {
      expect(await this.lpMakerV4.dao()).to.equal(this.dao.address);
    });
    ...
    it("should send 50% of fees to dao", async function () {
      ...
      expect(await this.usdt.balanceOf(this.lpMakerV4.address)).to.equal(0);
      await this.lpMakerV4.addLiquidityToStableSwap();
      await this.lpMakerV4.connect(this.owner).setProtocolOwnerLiquidityPercent(50);
      await this.lpMakerV4.sendLpToken();
      expect(await this.swapLPToken.balanceOf(this.dao.address)).to.be.closeTo("499974984710", 10);
      expect(await this.swapLPToken.balanceOf(this.pTRI.address)).to.be.closeTo("499974984710", 10);
    });
  });

  describe("StableLPMakerV4: onlyOwner tests", function () {
    it("only owner should be able to change pTRI addresses", async function () {
      await expect(this.lpMakerV4.connect(this.user1).setPTri(this.usdt.address)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
      await expect(this.lpMakerV4.connect(this.owner).setPTri(this.user1.address))
        .to.emit(this.lpMakerV4, "LogSetpTri")
        .withArgs(this.pTRI.address, this.user1.address);
      expect(await this.lpMakerV4.pTri()).to.equal(this.user1.address);
    });

    it("Only owner can change dao address", async function () {
      await expect(this.lpMakerV4.connect(this.user1).setDaoAddress(this.user1.address)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
      await this.lpMakerV4.connect(this.owner).setDaoAddress(this.user1.address);
      expect(await this.lpMakerV4.dao()).to.equal(this.user1.address);
      await this.lpMakerV4.connect(this.owner).setDaoAddress(this.dao.address);
      expect(await this.lpMakerV4.dao()).to.equal(this.dao.address);
    });

    it("should have correct pol percent", async function () {
      expect(await this.lpMakerV4.polPercent()).to.equal(0);
      await expect(this.lpMakerV4.connect(this.user1).setProtocolOwnerLiquidityPercent(49)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
      await expect(this.lpMakerV4.connect(this.owner).setProtocolOwnerLiquidityPercent(101)).to.be.revertedWith(
        "StableLPMaker: POL is too high",
      );
      await this.lpMakerV4.connect(this.owner).setProtocolOwnerLiquidityPercent(50);
      expect(await this.lpMakerV4.polPercent()).to.equal(50);
    });

    it("should be able to add and remove from the whitelist", async function () {
      // adding a stableswap
      expect(await this.lpMakerV4.whitelistedStableSwapAddresses(this.swapFlashLoan.address)).to.equal(false);
      await expect(this.lpMakerV4.connect(this.user1).addStableSwap(this.swapFlashLoan.address)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
      await this.lpMakerV4.connect(this.owner).addStableSwap(this.swapFlashLoan.address);
      expect(await this.lpMakerV4.whitelistedStableSwapAddresses(this.swapFlashLoan.address)).to.equal(true);
      ...
      // removing stableswap
      await expect(this.lpMakerV4.connect(this.user1).removeStableSwap(this.swapFlashLoan.address)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
      await this.lpMakerV4.connect(this.owner).removeStableSwap(this.swapFlashLoan.address);
      expect(await this.lpMakerV4.whitelistedStableSwapAddresses(this.swapFlashLoan.address)).to.equal(false);
    });
  });
});
