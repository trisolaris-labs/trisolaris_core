...
describe("TriMakerV2", function () {
  ...
  beforeEach(async function () {
    ...
    this.triMaker = await this.TriMaker.connect(this.minter).deploy(this.factory.address, this.bar.address, this.usdc.address, this.weth.address)
  ...
  describe("setBridge", function () {
    it("does not allow to set bridge for Usdc", async function () {
      await expect(this.triMaker.setBridge(this.usdc.address, this.weth.address)).to.be.revertedWith("TriMaker: Invalid bridge")
    })

    it("does not allow to set bridge for WETH", async function () {
      await expect(this.triMaker.setBridge(this.weth.address, this.usdc.address)).to.be.revertedWith("TriMaker: Invalid bridge")
    })
    ...
    it("does not allow to non_owner to set bridge", async function () {
      await expect(this.triMaker.connect(this.alice).setBridge(this.mic.address, this.dai.address)).to.be.revertedWith("Ownable: caller is not the owner")
    })
  ...
  describe("convert", function () {
    ...
    it("reverts if pair does not exist", async function () {
      await expect(this.triMaker.convert(this.mic.address, this.micTRI.address)).to.be.revertedWith("TriMaker: Invalid pair")
    })
    
    it("reverts if no path is available", async function () {
      await this.micTRI.connect(this.minter).transfer(this.triMaker.address, getBigNumber(1))
      await expect(this.triMaker.convert(this.mic.address, this.tri.address)).to.be.revertedWith("TriMaker: Cannot convert")
      expect(await this.usdc.balanceOf(this.triMaker.address)).to.equal(0)
  ...
  describe("setStableTriMaker", () => {
    it("should setStableTriMaker if owner", async function () {
