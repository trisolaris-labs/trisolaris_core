...
describe("MasterChefV2", function () {
  ...
  beforeEach(async function () {
    ...
    this.lp = await this.ERC20Mock.connect(this.minter).deploy("LPToken", "LPT", 18, getBigNumber(10));
    await this.lp.deployed();
    this.dummy = await this.ERC20Mock.connect(this.minter).deploy("Dummy", "DummyT", 18, getBigNumber(10));
    ...
    this.rlp = await this.ERC20Mock.connect(this.minter).deploy("RLP", "rLPT", 18, getBigNumber(100));
    await this.rlp.deployed();
    this.rlp2 = await this.ERC20Mock.connect(this.minter).deploy("RLP", "rLPT", 18, getBigNumber(100));
    await this.rlp2.deployed();
    this.rewardToken = await this.ERC20Mock.connect(this.minter).deploy("Reward", "RewardT", 18, getBigNumber(100000));
    await this.rewardToken.deployed();
    this.rewarder = await this.RewarderMock.connect(this.minter).deploy(
      1,
  ...
  describe("Set", function () {
    it("Should emit event LogSetPool", async function () {
      ...
      await expect(this.chefv2.set(0, 10, this.dummy.address, true))
        .to.emit(this.chefv2, "LogSetPool")
    ...
    it("Should revert if invalid pool", async function () {
      try {
        ...
        if (e instanceof Error) expect(e.toString()).to.not.equal("");
      }
    });
  });
  ...
  describe("PendingTri", function () {
    it("PendingTri should equal ExpectedTri", async function () {
      ...
      const log = await this.chefv2.connect(this.alice).deposit(0, getBigNumber(1), this.alice.address);
      await advanceBlock();
      const log2 = await this.chefv2.updatePool(0);
      await advanceBlock();
      const expectedTri = this.triPerBlockv1.mul(log2.blockNumber + 1 - log.blockNumber).div(2);
      const pendingTri = await this.chefv2.pendingTri(0, this.alice.address);
    ...
    it("When block is lastRewardBlock", async function () {
      ...
      const log = await this.chefv2.deposit(0, getBigNumber(1), this.alice.address);
      await advanceBlock();
      await advanceBlock();
      await advanceBlock();
      const log2 = await this.chefv2.updatePool(0);
      const expectedTri = this.triPerBlockv1.mul(log2.blockNumber - log.blockNumber).div(2);
      const pendingTri = await this.chefv2.pendingTri(0, this.alice.address);
      expect(pendingTri).to.be.equal(expectedTri);
    });
  });
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of TRI and reward", async function () {
      ...
      await advanceBlock();
      await advanceBlock();
      const log2 = await this.chefv2.connect(this.alice).withdraw(0, getBigNumber(1), this.alice.address);
      const expectedTri = this.triPerBlockv1.mul(log2.blockNumber - log.blockNumber).div(2);
      ...
      expect(triBal).to.be.equal(rewardBal).to.be.equal(expectedTri);
    });
    ...
    it("should give proper TRIs and rewardToken allocation to each pool", async function () {
      ...
      await this.chefv2.add("20", this.rlp2.address, this.rewarder.address);
