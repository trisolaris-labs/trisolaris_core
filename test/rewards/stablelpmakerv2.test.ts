...
describe("StableLpMaker", function () {
  ...
  beforeEach(async function () {
    ...
    this.lpMakerV2 = await this.LPMakerV2.connect(this.owner).deploy(
  ...
  describe("StableLPMakerV2: onlyOwner tests", function () {
    it("only owner should be able to change pTRI addresses", async function () {
      await expect(this.lpMakerV2.connect(this.user1).setPTri(this.usdt.address)).to.be.revertedWith(
    ...
    it("Only owner can change dao address", async function () {
      await expect(this.lpMakerV2.connect(this.user1).setDaoAddress(this.user1.address)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
    ...
    it("should have correct pol percent", async function () {
      ...
      await expect(this.lpMakerV2.connect(this.user1).setProtocolOwnerLiquidityPercent(49)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
      await expect(this.lpMakerV2.connect(this.owner).setProtocolOwnerLiquidityPercent(101)).to.be.revertedWith(
    ...
    it("should be able to add and remove from the whitelist", async function () {
      ...
      await expect(this.lpMakerV2.connect(this.user1).addStableSwap(this.swapFlashLoan.address)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
      ...
      // removing stableswap
      await expect(this.lpMakerV2.connect(this.user1).removeStableSwap(this.swapFlashLoan.address)).to.be.revertedWith(
        "Ownable: caller is not the owner",
      );
  ...
  describe("StableLPMakerV2 Unit Tests", function () {
    ...
    it("should convert USDC and USDT to USN ", async function () {
      ...
      await expect(this.lpMakerV2.swapStableTokens(this.swapFlashLoan.address, 0, 2)).to.be.revertedWith(
      ...
      await this.lpMakerV2.swapStableTokens(this.swapFlashLoan.address, 0, 2);
      ...
      await this.lpMakerV2.swapStableTokens(this.swapFlashLoan.address, 1, 2);
