...
describe("TriMaker", function () {
  ...
  beforeEach(async function () {
    this.tri = await this.ERC20Mock.connect(this.minter).deploy("TRI", "TRI", getBigNumber("10000000"))
    await this.tri.deployed()
    this.dai = await this.ERC20Mock.connect(this.minter).deploy("DAI", "DAI", getBigNumber("10000000"))
    await this.dai.deployed()
    this.mic = await this.ERC20Mock.connect(this.minter).deploy("MIC", "MIC", getBigNumber("10000000"))
    await this.mic.deployed()
    this.usdc = await this.ERC20Mock.connect(this.minter).deploy("USDC", "USDC", getBigNumber("10000000"))
    await this.usdc.deployed()
    this.weth = await this.ERC20Mock.connect(this.minter).deploy("WETH", "ETH", getBigNumber("10000000"))
    await this.usdc.deployed()
    this.strudel = await this.ERC20Mock.connect(this.minter).deploy("$TRDL", "$TRDL", getBigNumber("10000000"))
