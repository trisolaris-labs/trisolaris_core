...
contract StableLPMakerV4 is Ownable {
    ...
    ISwap public twoPoolStableSwap;
    address public pTri;
    ...
    event LogAddliquidityToStableSwap(uint256 usdcAmount, uint256 usdtAmount);

    event LogLpTokensSentTopTRI(uint256 tlpAmount);
    event LogLpTokensSentToDao(uint256 daoAmount);

    constructor(
        address _twoPoolStableSwap,
        address _pTri,
        address _usdt,
        address _usdc,
        address _tlpToken,
        ...
        twoPoolStableSwap = ISwap(_twoPoolStableSwap);
        pTri = _pTri;
        usdt = _usdt;
        usdc = _usdc;
        tlpToken = _tlpToken;
    ...
    */
    ...
    function addLiquidityToStableSwap() public onlyEOA {
        ...
        require((usdcAmount > 0 || usdtAmount > 0), "StableLPMaker: no liquidity to add");

        // approve transferFrom
        IERC20(usdc).approve(address(twoPoolStableSwap), usdcAmount);
        IERC20(usdt).approve(address(twoPoolStableSwap), usdtAmount);

        // add liquidity
        uint256[] memory tokenAmounts = new uint256[](2);
        tokenAmounts[twoPoolStableSwap.getTokenIndex(usdc)] = usdcAmount;
        tokenAmounts[twoPoolStableSwap.getTokenIndex(usdt)] = usdtAmount;
        twoPoolStableSwap.addLiquidity(tokenAmounts, 0, block.timestamp + 60);

        emit LogAddliquidityToStableSwap(usdcAmount, usdtAmount);
    }
    ...
    function convertStables(
        ...
        // convert set of stable tokens to usdc, usdt
