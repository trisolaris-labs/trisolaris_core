...
async function main(): Promise<void> {
  ...
  let canAddPool = true;
        ...
        if (canAddPool) {
            ...
            const tx = await chef.connect(deployer).add(allocPoint, lpAddress, rewarderAddress, false);
            console.log(tx)
  ...
  await dummyLP.connect(deployer).approve(chefv2.address, dummyLPBalance);
